...
var newsController = function newsController($scope, $state, dashboardService, toaster) {
    var _this = this;
    ...
    $scope.func.todo = function () {
        try {
            ...
            dashboardService.dashboard.getList("getUserSearchs").then(function () {
                                ...
                                case 0:
                                    ...
                                    if (_.isArray(res.data.originalElement) && res.data.originalElement.length > 0) {
                                        _.map(res.data.originalElement, function (item) {
                                            ...
                                            item.properiesList = [];
                                            if (item.properties && !_.isEqual(item.properties, {})) {
                                                _.forEach(item.properties, function (v, k) {
                                                    item.properiesList.push({
                                                        key: k,
                                                        value: v.term
                                                    });
                                                });
    ...
    $scope.func.go2Advance = function () {
            ...
            return regeneratorRuntime.wrap(function _callee2$(_context2) {
                while (1) {
                    switch (_context2.prev = _context2.next) {
                        ...
                        case 8:
                            _context2.next = 10;
                            return setStateVarFunction({ advance: item });

                        case 10:
                            $state.go('main.page.advance', {}, { reload: false });
                            _context2.next = 16;
                            break;
...
newsController.$inject = ['$scope', '$state', 'dashboardService', 'toaster'];