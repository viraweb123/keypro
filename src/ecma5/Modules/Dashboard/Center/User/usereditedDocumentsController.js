...
var usereditedDocumentsController = function usereditedDocumentsController($scope, $state, dashboardService, toaster, $filter) {
    var _this = this;

    _classCallCheck(this, usereditedDocumentsController);
    ...
    $scope.func.convertToCSV = _asyncToGenerator(regeneratorRuntime.mark(function _callee2() {
        var json, i, result;
            ...
            while (1) {
                switch (_context2.prev = _context2.next) {
                    case 0:
                        ...
                        console.log(angular.copy($scope.data.list));
                        ...
                        for (i = 0; i < $scope.data.list.length; i++) {
                            json.push({
                                'عنوان': $filter('pNumber')(angular.copy($scope.data.list[i].document.title)),
                                'ایجاد کننده': $filter('pNumber')(angular.copy($scope.data.list[i].document.author)),
                                'تاریخ ایجاد': $filter('pNumber')($filter('persianDate')(angular.copy($scope.data.list[i].document.created), 'yyyy/MM/dd')),
                                'تاریخ آخرین تغییر': $filter('pNumber')($filter('persianDate')(angular.copy($scope.data.list[i].document.lastModified), 'yyyy/MM/dd')),
                                'اندازه سند': $filter('pNumber')($filter('fileSize')(angular.copy($scope.data.list[i].document.actualVersion.size))),
                                'نسخه': $filter('pNumber')(angular.copy($scope.data.list[i].document.actualVersion.name)),
                                'آدرس': $filter('pNumber')(angular.copy($scope.data.list[i].document.lockInfo.nodePath))
                            });
                        }
                        ...
                        _context2.next = 7;
                        return JSManagement.addJS("ecma5/ExternalLiberary/Public/json2CSVConvertor.js?dev=" + randomVersionName);

                    case 7:
                        _context2.next = 9;
                        return json2CSVConvertorFunction(angular.copy(json), 'لیست اسناد ویرایش شده');
                    ...
                    case 17:
                        _context2.prev = 17;
                        _context2.next = 20;
                        return JSManagement.removeJS("ecma5/ExternalLiberary/Public/json2CSVConvertor.js?dev=" + randomVersionName, 'json2CSVConvertorFunction');
    ...
    $scope.func.todo = function () {
        try {
            ...
            dashboardService.dashboard.getList("getUserCheckedOutDocuments").then(function () {
    ...
    $scope.func.go2Desktop = function () {
        var _ref4 = _asyncToGenerator(regeneratorRuntime.mark(function _callee4(item) {
                ...
                while (1) {
                    switch (_context4.prev = _context4.next) {
                        ...
                        case 19:
                            ...
                            toaster.pop("error", "", _context4.t1.message);
            ...
            }, _callee4, _this, [[0, 19], [3, 11, 14, 17]]);
        }));
...
exports.default = usereditedDocumentsController;


usereditedDocumentsController.$inject = ['$scope', '$state', 'dashboardService', 'toaster', '$filter'];