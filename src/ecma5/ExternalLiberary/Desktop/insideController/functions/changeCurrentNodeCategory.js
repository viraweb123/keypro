...
var changeCurrentNodeCategoryDesktopCtrl = function changeCurrentNodeCategoryDesktopCtrl($scope, desktopService, JSManagement, CSSManagement, toaster, $uibModal, userInfoService, Authentication) {
                        ...
                        while (1) {
                            switch (_context.prev = _context.next) {
                                ...
                                case 8:
                                    ...
                                    if ($scope.data.trees.categoryTree.currentNode.hasChildrenFolder) desktopService.folder.listChild($scope.data.trees.categoryTree.currentNode.id).then(function (result) {
                                        if (result.data != null) {
                                            ...
                                            _.forEach(result.data, function (res) {
                                                if (_.has(res, 'uuid') || _.has(res, 'id')) {
                                                    //if (_.findIndex(_.map($scope.data.trees.categoryTree.currentNode.children, 'id'), id => id == (res.uuid || res.id)) == -1) {
                                                    if (typeof $scope.data.pointers["categoryVisible"][res.uuid || res.id] === "undefined") {
                                                        ...
                                                        $scope.data.pointers["categoryVisible"][res.uuid || res.id] = $scope.data.trees.categoryTree.currentNode.children.slice(-1)[0];
                                                        $scope.data.nodesFolder[res.path] = $scope.data.pointers["categoryVisible"][res.uuid || res.id];
                                                    }
                                                }
                                            });
                                        } else $scope.data.trees.category.currentNodeCount = 0;
                                        $scope.func.selectFolder($scope.data.trees.categoryTree.currentNode, false, viewDoc);
                                    }, function (error) {
                                        return console.error(error);
                                    });else $scope.func.selectFolder($scope.data.trees.categoryTree.currentNode, false, viewDoc);
                                    /*$scope.func.stopAllPlayers();
