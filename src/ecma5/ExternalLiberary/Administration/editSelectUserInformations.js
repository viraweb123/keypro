...
var editSelectUserInformations = function editSelectUserInformations(uibModal, _user, _JS, _service) {
    return new Promise(function (resolve, reject) {
        var _this = this;

        var modalInstance = uibModal.open({
            templateUrl: 'ecma5/ExternalLiberary/Administration/editSelectUserInformations.html?dev=' + randomVersionName,
            controller: function controller($scope, $uibModalInstance, toaster, $uibModal, user, JS, service, Roles, profiles) {
                ...
                $scope.data = {
                    ...
                    txtEmailname: user.Email,
                    txtActive: user.Active,
                    name: user.UserId,

                    removeExternalFuncs: {},

                    txtProfile: _.has(user, 'Profile') ? user.Profile : "",
                ...
                _.forEach(Roles, function (role) {
                    $scope.data.roles.push({
                        name: role.id,
                        selectRole: _.includes(user.Roles, role.id)
                    });
                });
                ...
                $scope.func.edit = function () {
                    ...
                    if (_.includes([null, ""], $scope.data.txtProfile)) {
                        toaster.pop('error', "", "لطفا یک پروفایل انتخاب کنید");
                        return;
                    }
