...
var selectPathFromTree = function selectPathFromTree(uibModal, repository, rootPath, _folder, _getNameFromPath) {
    return new Promise(function (resolve, reject) {

        var modalInstance = uibModal.open({
            templateUrl: 'ecma5/ExternalLiberary/Public/selectPathFromTree.html?dev=' + randomVersionName,
            controller: function controller($scope, $uibModalInstance, root, folder, getNameFromPath) {
                ...
                $scope.func.copy = function () {
                    if ($scope.data.copyTree.currentNode) $uibModalInstance.close($scope.data.copyTree.currentNode.path);else $uibModalInstance.close(false);
                };
            },
            size: 'md',
