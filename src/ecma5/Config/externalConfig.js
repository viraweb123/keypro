        ...
        value: function runAPP($rootScope, Restangular, toaster, $state, $http, $q, Authentication) {
            ...
            $(document).ready(function () {
                function copyToClipboard() {
                    ...
                    toaster.pop("error", "", "امکان گرفتن تصویر از صفحه فوق نیست .");
                    _.defer(function () {
            ...
            var newAuthorizationRequest = function newAuthorizationRequest(element, operation, route, url, headers, params, httpConfig, cookieExist) {
                return new Promise(function (resolve, reject) {
                    $.ajax({
                        url: webAppTypingFormat.getUserFromToken + "/" + cookieExist,
                        type: "GET",
                        jsonpCallback: 'jsonCallback',
                        dataType: "json",
                        cache: 'false',
                        success: function success(data) {
                            var encode = _encodeBase(data.username + ":" + cookieExist);
                            $.ajax({
                                ...
                                cache: 'false',
                                success: function success(data) {
                                    createCookiesWithNameFunction('Authorization', "Bearer" + encode).then(function (success) {
                                        var AuthName = isCookieFunction("Authorization");
                                        resolve({
                                            element: element,
                                            params: params,
                                            headers: AuthName != null ? _.extend(headers || {}, { Authorization: decodeURIComponent('Bearer ' + AuthName) }) : headers, httpConfig: httpConfig || {}
                                        });
                                    }, function (err) {
                                        console.error(err);
                                        reject("error");
            ...
            Restangular.addFullRequestInterceptor(function () {
                var _ref2 = _asyncToGenerator(regeneratorRuntime.mark(function _callee2(element, operation, route, url, headers, params, httpConfig) {
                    var defer, header, AuthName, cookieExist;
                        ...
                        while (1) {
                            switch (_context2.prev = _context2.next) {
                                case 0:
                                    ...
                                    defer = $q.defer();
                                    header = headers;
                                    //let isValidRequestted = _.random(1,10);
                                    ...
                                    cookieExist = isCookieFunction(webAppTypingFormat.SsoTokenName);
                                    _context2.prev = 9;
                                    ...
                                    _context2.next = 13;
                                    return newAuthorizationRequest(element, operation, route, url, headers, params, httpConfig, cookieExist);
                                ...
                                case 19:
                                    ...
                                    window.location = webAppTypingFormat.redirect(window.location.href);
                                ...
                                case 25:
                                    ...
                                    /*if(isReadyGettingFromServer  && isValidRequestted>=9){
                                        httpConfig.timeout = defer.promise;
                                        defer.resolve();
                                    }
                                    else{
