    ...
    _createClass(Authorization, [{
        ...
        value: function setAuthenticate(username, password, $http, authService, expiresTimeig) {
            ...
            return new Promise(function(resolve, reject) {
                try {
                    (function() {
                        var item = _Base2.default.encode(username + ':' + password);
                        try {

                            console.info("Expire******************************************************************************");

                            createCookiesWithNameFunction('Authorization', 'Bearer' + item).then(function() {
        ...
        value: function modal(resolve, reject) {
                        ...
                        sendLoginRequest: function sendLoginRequest() {
                            return new Promise(function(resolve, reject) {
                                auth.login($scope.data.txtUsername, $scope.data.txtPassword).then(function(authResult) {
                                    setAuthenticate($scope.data.txtUsername, $scope.data.txtPassword, $http, auth, expiresTimeLefy).then(function(authenticationResult) {
            ...
            this.modalInstance.result.then(function(response) {
                if (response) {
                    _this4.Promise = null;
                    _this4.$rootScope.$broadcast("changeFeature", {
                        changeInfo: true
                    });
                    resolve(response);
                } else {
