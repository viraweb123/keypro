...
function ($compile, $parse, $document, $uibPosition, dateFilter, uibDateParser, datepickerPopupConfig, PersianDateService, persianDateFilter,EnToFaNumberFilter) {
  return {
    restrict: 'EA',
    require: 'ngModel',
    scope: {
    ...
    link: function(scope, element, attrs, ngModel) {
      ...
      function parseDate(viewValue) {
          if (!viewValue) {
            ...
            //var date = uibDateParser.parse(viewValue, dateFormat) || new Date(viewValue);
          	var dateArray=viewValue.split("/")
            var date = PersianDateService.persian_to_gregorian_Date(parseInt(dateArray[0]),parseInt(dateArray[1])-1,parseInt(dateArray[2])) || new Date(viewValue);
            console.log(data);
            if (isNaN(date)) {
      ...
      ngModel.$formatters.push(function (value) {
    	  ...
    	  return ngModel.$isEmpty(value) ? value : EnToFaNumberFilter(persianDateFilter(value, dateFormat));
    	});
      // Inner change
...
angular.module("template/persianDatepicker/day.html", []).run(["$templateCache", function($templateCache) {
  $templateCache.put("template/persianDatepicker/day.html",
    ...
    "  <thead>\n" +
    "    <tr>\n" +
    "      <th style=\"padding:0px 3px !important;\"><button type=\"button\" class=\"btn btn-default btn-sm pull-right\" ng-click=\"move(-1)\" tabindex=\"-1\"><i class=\"fa fa-chevron-right\"></i></button></th>\n" +
    ...
    "      <th style=\"padding:0px 3px !important;\"><button type=\"button\" class=\"btn btn-default btn-sm pull-left\" ng-click=\"move(1)\" tabindex=\"-1\"><i class=\"fa fa-chevron-left\"></i></button></th>\n" +
    "    </tr>\n" +
    "    <tr style=\" height: 15px;font-size: 8px;\">\n" +
    ...
    "      <td ng-show=\"showWeeks\" style=\"width:25px;font-size:14px;\" class=\"text-center\"><em>{{ weekNumbers[$index] |EnToFaNumber }}</em></td>\n" +
...
angular.module("template/persianDatepicker/month.html", []).run(["$templateCache", function($templateCache) {
  $templateCache.put("template/persianDatepicker/month.html",
    ...
    "  </tbody>\n" +
    "</table>\n" +
    "");//min-width:200px;position:inherit;top:-45px;
}]);
...
angular.module("template/persianDatepicker/popup.html", []).run(["$templateCache", function($templateCache) {
  $templateCache.put("template/persianDatepicker/popup.html",
      ...
      "<ul style=\"direction:rtl;overflow-x:auto;padding:5px;position:absolute;\"  class=\"dropdown-menu datepicker-popup\" ng-style=\"{display: (isOpen && 'block') || 'none' }\" ng-keydown=\"keydown($event)\">\n" +
      "	<li ng-transclude  style=\"padding: 5px 0px;\"></li>\n" +
      "	<li ng-if=\"showButtonBar\" style=\"padding:auto\">\n" +
      "		<span class=\"btn-group  col-sm-12\">\n" +
      "			<button type=\"button\" class=\"btn btn-sm btn-info\" style=\"width: 30px;\" ng-click=\"select('today')\"><span class=\"fa fa-calendar-o\"></span><md-tooltip  style=\"z-index:10000000000 !important;padding : 0px 12px;\" md-direction=\"bottom\">امروز</md-tooltip></button>\n" +
      "			<button type=\"button\" class=\"btn btn-sm btn-danger\" style=\"width: 30px;\" ng-click=\"select(null)\"><span class=\"fa fa-trash\"></span><md-tooltip  style=\"z-index:10000000000 !important;padding : 0px 12px;\" md-direction=\"bottom\">پاک کردن</md-tooltip></button>\n" +
      "		    <button type=\"button\" class=\"btn btn-sm btn-success  pull-left \" ng-click=\"close()\" style=\"width: 30px;\"><span class=\"fa fa-remove\"></span><md-tooltip  style=\"z-index:10000000000 !important;padding : 0px 12px;\" md-direction=\"bottom\">بستن پنجره</md-tooltip></button>\n" +
      "		</span>\n" +
      "	</li>\n" +
      "</ul>\n" +
      "");
...
angular.module("template/persianDatepicker/year.html", []).run(["$templateCache", function($templateCache) {
  $templateCache.put("template/persianDatepicker/year.html",
    "<table  class=\"datepicker-table-viewer\" role=\"grid\" aria-labelledby=\"{{uniqueId}}-title\" aria-activedescendant=\"{{activeDateId}}\">\n" +
    "  <thead>\n" +
    "    <tr>\n" +
    "      <th><button type=\"button\" class=\"btn btn-default btn-sm pull-right\" ng-click=\"move(-1)\" tabindex=\"-1\"><i class=\"fa fa-chevron-right\"></i></button></th>\n" +
